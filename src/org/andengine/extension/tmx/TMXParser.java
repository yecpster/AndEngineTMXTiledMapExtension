...
public class TMXParser extends DefaultHandler implements TMXConstants {
    ...
    public void startElement(final String pUri, final String pLocalName, final String pQualifiedName, final Attributes pAttributes) throws SAXException {
        if (pLocalName.equals(TMXConstants.TAG_MAP)) {
            ...
            if (tsxTileSetSource == null) {
                ...
                try {
                    ...
                    final TSXLoader tsxLoader = new TSXLoader(this.mAssetManager, this.mTextureManager, this.mTextureOptions, mTMXTiledMap, bitmapTextureFormat);
            ...
            final TMXTileSet tmxTileSet = tmxTileSets.get(tmxTileSets.size() - 1);
            tmxTileSet.setImageSource(this.mAssetManager, this.mTextureManager, pAttributes);
        ...
        } else if (pLocalName.equals(TMXConstants.TAG_TERRAIN_TYPES) || pLocalName.equals(TMXConstants.TAG_TERRAIN)) {
            // Ignroe
        } else {
    ...
    public void endElement(final String pUri, final String pLocalName, final String pQualifiedName) throws SAXException {
        if (pLocalName.equals(TMXConstants.TAG_MAP)) {
            ...
            if (binarySaved) {
                final ArrayList<TMXLayer> tmxLayers = this.mTMXTiledMap.getTMXLayers();
                try {
                    tmxLayers.get(tmxLayers.size() - 1).initializeTMXTilesFromDataString(this.mStringBuilder.toString().trim(), this.mDataEncoding,
                            this.mDataCompression, this.mTMXTilePropertyListener);
